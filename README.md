...
| テスト | PHPUnit（バックエンド Feature テスト、100件） |

## 主な機能

### 購入者向け

- 会員登録・ログイン・ログアウト・パスワード変更・パスワード再設定（メールのトークンリンク）
...
### 管理者向け（`role: admin`のユーザーのみ、ログイン後は自動でダッシュボードに遷移）

- ダッシュボード（今月の売上・注文件数・会員数・在庫切れ件数、売上推移グラフ（時間帯/週/月/年の切替・カテゴリ別+合計の折れ線）、カテゴリ別売上比率、問い合わせ状況、最近の注文）
...
- 問い合わせ管理：一覧（未読/既読/回答済みで絞り込み）・詳細（開くと既読になる）・返信（顧客のメールアドレス宛にメール送信）
